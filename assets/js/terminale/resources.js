 ...
 "term-structures": {
  ...
  "seances": [
   ...
   },
   {
    ...
    "cours": "<p><strong>📖 La notion :</strong></p><p>La <code>list</code> de Python range ses éléments côte à côte en mémoire : pour insérer en tête, il faut tout décaler. La <strong>liste chaînée</strong> organise les données autrement : une suite de <strong>maillons</strong>, chaque maillon contenant une <strong>valeur</strong> et une <strong>flèche</strong> vers le maillon suivant, le dernier pointant vers « rien » (<code>None</code>).</p><p>À la main : la chaîne 1 → 2 → 3 → None. Pour insérer 0 en tête, on crée un maillon 0 dont la flèche vise l'ancienne tête : 0 → 1 → 2 → 3 → None. <strong>Aucun décalage</strong> — mais pour atteindre le 3, il faut suivre les flèches une à une : pas d'accès direct par indice. La définition est naturellement <strong>récursive</strong> : une liste chaînée est <em>soit vide</em>, <em>soit</em> un maillon suivi d'une liste chaînée.</p><p><strong>🐢 Première méthode — simple à comprendre :</strong> la classe <code>Maillon</code> et la longueur récursive.</p><pre><code>class Maillon:\n    def __init__(self, valeur, suivant=None):\n        self.valeur = valeur\n        self.suivant = suivant   # flèche vers le maillon d'après (ou None)\n\ntete = Maillon(1, Maillon(2, Maillon(3)))   # la chaîne 1 -&gt; 2 -&gt; 3\n\ndef longueur(maillon):\n    if maillon is None:          # cas de base : liste vide\n        return 0\n    return 1 + longueur(maillon.suivant)\n\nprint(longueur(tete))     # affiche 3\ntete = Maillon(0, tete)   # insertion en tête : on rebranche une flèche\nprint(longueur(tete))     # affiche 4</code></pre><p><strong>🔍 Comment ça marche :</strong></p><ul><li><code>suivant=None</code> par défaut : un maillon créé seul est un dernier maillon.</li><li><code>longueur</code> suit la définition récursive : liste vide, 0 maillon ; sinon 1 (le maillon courant) plus la longueur du reste. Sans le cas de base <code>None</code>, la récursion serait infinie.</li><li>L'insertion en tête crée un maillon et rebranche une flèche : coût O(1), contre O(n) pour une <code>list</code>.</li></ul><p><strong>📋 Trace d'exécution :</strong> <code>longueur(tete)</code> sur la chaîne 1 → 2 → 3.</p><table><tr><th>Appel</th><th>Cas</th><th>Renvoie</th></tr><tr><td>longueur(maillon 1)</td><td>appel récursif</td><td>1 + longueur(maillon 2)</td></tr><tr><td>longueur(maillon 2)</td><td>appel récursif</td><td>1 + longueur(maillon 3)</td></tr><tr><td>longueur(maillon 3)</td><td>appel récursif</td><td>1 + longueur(None)</td></tr><tr><td>longueur(None)</td><td>cas de base</td><td>0</td></tr></table><p>La remontée donne 1, puis 2, puis 3 : la chaîne compte bien 3 maillons.</p><p><strong>🧩 Le projet de la séance :</strong> concevoir une structure absente de Python, la <strong>file de priorité</strong>. La vie scolaire note des tâches avec une priorité de 1 (peut attendre) à 5 (urgent) ; l'outil doit toujours proposer la tâche la plus urgente et, à priorité égale, la plus ancienne. Ni une pile (LIFO) ni une file (FIFO) ne conviennent. On définit d'abord l'<em>interface</em> — <code>ajouter(description, priorite)</code>, <code>suivante()</code>, <code>est_vide()</code> — puis on implémente la classe <code>GestionnaireTaches</code> avec des tuples <code>(priorite, numero, description)</code>. Deux implémentations possibles : chercher le maximum au moment de <code>suivante()</code>, ou maintenir la liste triée dès <code>ajouter()</code> — même interface, coûts différents.</p><p><strong>🎯 Défi élève :</strong> compléter la chaîne et sa longueur.</p><pre><code>class Maillon:\n    def __init__(self, valeur, suivant=______):\n        self.valeur = valeur\n        self.suivant = suivant\n\ndef longueur(maillon):\n    if maillon is ______:\n        return ______\n    return 1 + longueur(maillon.______)\n\ntete = Maillon(1, Maillon(2, Maillon(3)))\ntete = Maillon(0, ______)   # insérer 0 en tête\nprint(longueur(tete))       # affiche 4</code></pre><p><strong>✅ Réponse :</strong></p><pre><code>class Maillon:\n    def __init__(self, valeur, suivant=None):\n        self.valeur = valeur\n        self.suivant = suivant\n\ndef longueur(maillon):\n    if maillon is None:\n        return 0\n    return 1 + longueur(maillon.suivant)\n\ntete = Maillon(1, Maillon(2, Maillon(3)))\ntete = Maillon(0, tete)     # insérer 0 en tête\nprint(longueur(tete))       # affiche 4</code></pre><ul><li>Liste chaînée : insertion en tête en O(1), mais pas d'accès direct par indice — on suit les flèches une à une.</li><li>Toute fonction récursive sur une chaîne commence par le cas de base <code>maillon is None</code> ; l'oublier provoque une récursion infinie.</li><li>File de priorité : ce n'est ni l'ordre d'arrivée ni l'ordre inverse qui commande, c'est l'<strong>urgence</strong> — et l'interface reste la même quel que soit le choix d'implémentation.</li></ul>"
   },
   {
    ...
    "cours": "<p><strong>📖 La notion :</strong></p><p>La <code>list</code> de Python range ses éléments côte à côte en mémoire : pour insérer au début, il faut tout décaler. Une <strong>liste chaînée</strong> organise les données autrement : une suite de <strong>maillons</strong>, où chaque maillon contient une <strong>valeur</strong> et une <strong>flèche</strong> vers le maillon suivant ; le dernier pointe vers « rien » (<code>None</code>).</p><p>Exemple à la main : la chaîne 1 → 2 → 3. Le maillon de tête porte la valeur 1 et une flèche vers le maillon 2, qui pointe vers le maillon 3, qui pointe vers <code>None</code>. Pour insérer 0 en tête, on crée un maillon 0 dont la flèche vise l'ancienne tête : 0 → 1 → 2 → 3. <strong>Aucun décalage</strong>, une seule flèche à brancher. En revanche, pour atteindre le troisième élément, on suit les flèches une à une : <strong>pas d'accès direct par indice</strong>.</p><p>D'où la définition <strong>récursive</strong> (lien avec la récursivité du thème Langages) : une liste chaînée est <em>soit vide</em>, <em>soit</em> un maillon suivi d'une liste chaînée.</p><p><strong>🐢 Première méthode — simple à comprendre :</strong></p><pre><code>class Maillon:\n    def __init__(self, valeur, suivant=None):\n        self.valeur = valeur\n        self.suivant = suivant   # flèche vers le maillon d'après (ou None)\n\ntete = Maillon(1, Maillon(2, Maillon(3)))   # la chaîne 1 -&gt; 2 -&gt; 3\n\ndef longueur(maillon):\n    if maillon is None:              # cas de base : liste vide\n        return 0\n    return 1 + longueur(maillon.suivant)\n\nprint(longueur(tete))   # affiche 3</code></pre><p><strong>🔍 Comment ça marche :</strong></p><ul><li><code>suivant=None</code> par défaut : un maillon créé seul est un dernier maillon ; <code>Maillon(1, Maillon(2, Maillon(3)))</code> construit donc la chaîne de la fin vers la tête.</li><li><code>longueur</code> suit mot à mot la définition récursive : liste vide → 0 ; sinon 1 (ce maillon) plus la longueur du reste.</li><li>Insérer en tête tient en une ligne : <code>tete = Maillon(0, tete)</code> — exactement l'opération que la <code>list</code> paie au prix fort.</li></ul><p><strong>📋 Trace d'exécution :</strong> déroulé de <code>longueur(tete)</code> sur la chaîne 1 → 2 → 3 :</p><table><tr><th>Appel</th><th>maillon</th><th>Renvoie</th></tr><tr><td>longueur(tete)</td><td>valeur 1</td><td>1 + longueur(maillon 2)</td></tr><tr><td>longueur(maillon 2)</td><td>valeur 2</td><td>1 + longueur(maillon 3)</td></tr><tr><td>longueur(maillon 3)</td><td>valeur 3</td><td>1 + longueur(None)</td></tr><tr><td>longueur(None)</td><td>vide</td><td>0 (cas de base)</td></tr></table><p>À la remontée : 1 + (1 + (1 + 0)) = 3.</p><p><strong>📌 Le projet en îlots — la file de priorité :</strong></p><p>La vie scolaire veut un outil qui propose toujours <strong>la tâche la plus urgente</strong> (priorité de 1 à 5) et, à priorité égale, la plus ancienne. Ni une pile (LIFO) ni une file (FIFO) ne conviennent : l'ordre de sortie ne dépend pas de l'ordre d'arrivée. Il faut concevoir une structure absente de Python : la <strong>file de priorité</strong>, d'interface <code>ajouter(description, priorite)</code>, <code>suivante()</code>, <code>est_vide()</code>. Implémentation retenue : une liste de tuples <code>(priorite, numero, description)</code>, où le compteur <code>numero</code> départage l'ancienneté. Deux choix possibles, à comparer : chercher le maximum au moment de <code>suivante()</code> (ajout rapide, retrait en O(n)) ou maintenir la liste triée dès <code>ajouter()</code>.</p><p><strong>🎯 Défi élève :</strong> compléter la classe <code>GestionnaireTaches</code> (quatre trous).</p><pre><code>class GestionnaireTaches:\n    def __init__(self):\n        self.taches = []      # tuples (priorite, numero, description)\n        self.numero = 0       # compteur d'arrivée\n\n    def est_vide(self):\n        return self.taches == ______\n\n    def ajouter(self, description, priorite):\n        self.numero += 1\n        self.taches.______((priorite, self.numero, description))\n\n    def suivante(self):\n        assert not self.est_vide(), \"aucune tâche en attente !\"\n        meilleure = 0\n        for i in range(len(self.taches)):\n            prio_i, num_i, _ = self.taches[i]\n            prio_m, num_m, _ = self.taches[meilleure]\n            if prio_i ______ prio_m or (prio_i == prio_m and num_i &lt; num_m):\n                meilleure = i\n        return self.taches.pop(meilleure)[______]</code></pre><p><strong>✅ Réponse :</strong></p><pre><code>class GestionnaireTaches:\n    def __init__(self):\n        self.taches = []      # tuples (priorite, numero, description)\n        self.numero = 0       # compteur d'arrivée\n\n    def est_vide(self):\n        return self.taches == []\n\n    def ajouter(self, description, priorite):\n        self.numero += 1\n        self.taches.append((priorite, self.numero, description))\n\n    def suivante(self):\n        assert not self.est_vide(), \"aucune tâche en attente !\"\n        meilleure = 0\n        for i in range(len(self.taches)):\n            prio_i, num_i, _ = self.taches[i]\n            prio_m, num_m, _ = self.taches[meilleure]\n            if prio_i &gt; prio_m or (prio_i == prio_m and num_i &lt; num_m):\n                meilleure = i\n        return self.taches.pop(meilleure)[2]\n\ng = GestionnaireTaches()\ng.ajouter(\"changer une ampoule\", 2)\ng.ajouter(\"réparer le vidéoprojecteur\", 5)\ng.ajouter(\"ranger la réserve\", 2)\nprint(g.suivante())   # affiche réparer le vidéoprojecteur\nprint(g.suivante())   # affiche changer une ampoule\nprint(g.suivante())   # affiche ranger la réserve\nprint(g.est_vide())   # affiche True</code></pre><ul><li>Liste chaînée : insertion en tête immédiate, mais pas d'accès direct par indice — l'inverse de la <code>list</code>.</li><li>Oublier le cas de base (<code>maillon is None</code>) provoque une récursion infinie.</li><li>L'interface est le contrat ; chaque îlot choisit son implémentation et doit savoir justifier son coût.</li></ul>"
   },
   {
