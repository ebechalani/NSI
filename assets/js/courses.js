...
`,
      ...
      },
      {
        ...
        <p class="note">🗺️ <strong>Par où continuer ?</strong> Le site <a href="https://roadmap.sh" target="_blank" rel="noopener">roadmap.sh</a> propose des <strong>feuilles de route</strong> visuelles, étape par étape, pour apprendre un domaine — par exemple la roadmap <a href="https://roadmap.sh/frontend" target="_blank" rel="noopener">Frontend</a> (HTML/CSS/JS → frameworks) ou <a href="https://roadmap.sh/javascript" target="_blank" rel="noopener">JavaScript</a>. Une bonne carte pour visualiser ce qu'il reste à découvrir après ce chapitre. <em>(En anglais.)</em></p>`,
      },
      {
...
# C'est tout l'intérêt du projet « Recherche séquentielle vs dichotomique ».`,
         ...
         adaptés, texte original. ---- */
      {
        ...
        <p class="warnbox">⚠️ kNN ne « voit » que la taille et le poids. Un centre de 106 kg ressemble à un avant : l'algorithme se trompe sur lui, et cette « erreur » en dit plus sur les <strong>données choisies</strong> que sur l'algorithme.</p>
...
for joueur in test:
    ...
    print(joueur[0].ljust(10), joueur[1], "cm", joueur[2], "kg ->", prediction.ljust(7), verdict)
    reussites += prediction == joueur[3]
print("Taux de réussite sur le jeu de test :", reussites, "/", len(test))   # 7 / 8`,
        prof:
      ...
      },
      {
        ...
        <p class="note">📎 Source : d'après le diaporama « K plus proches voisins (KNN) — Introduction au Machine Learning » de <strong>Laurent Amanton</strong> (DIU EIL, Université Le Havre Normandie, 2021-2026), disponible dans « Pour aller plus loin » en bas du thème. Le jeu de données du XV de France est celui de mon TP.</p>`,
        code: `# 1) La distance euclidienne se généralise à n caractéristiques (Pythagore en n dimensions)
def euclidienne(a, b):
    return sum((a[i] - b[i]) ** 2 for i in range(len(a))) ** 0.5

# 2) La distance de Manhattan : on additionne les écarts (comme dans les rues d'une ville en damier)
def manhattan(a, b):
...
# 3) Le piège des échelles : clients d'une banque (âge, revenu annuel en €, nb de crédits) -> a remboursé ?
clients = [
...
print("Sans normalisation :", knn(clients, inconnu, euclidienne))   # oui : le revenu écrase tout
...
print("Avec normalisation :", knn(clients_n, inconnu_n, euclidienne))   # non : les 4 crédits comptent enfin`,
        prof:
...
scaler = StandardScaler().fit(X_train)          # normalisation (variante du min-max)
...
print(accuracy_score(y_test, y_pred))           # taux de réussite (souvent > 0.9)</code></pre>
        <p>Tu reconnais chaque étape du thème : découpage entraînement / test, normalisation, choix de k, prédiction, mesure. Ce que tu as codé toi-même, c'est exactement ce que la bibliothèque fait, en plus rapide et en plus général.</p>
        <p class="note">📌 Hors programme de Première : scikit-learn, iris et le vocabulaire sur-apprentissage / biais-variance sont là pour la culture, un projet personnel ou un futur Grand oral. Ce qui est attendu au BO : écrire l'algorithme kNN (sections 7 et 11 à 13) et comprendre le rôle de k.</p>
        <p class="note">📎 Source : d'après le diaporama « K plus proches voisins (KNN) — Introduction au Machine Learning » de <strong>Laurent Amanton</strong> (DIU EIL, Université Le Havre Normandie, 2021-2026), disponible dans « Pour aller plus loin » en bas du thème. Le jeu de données du XV de France est celui de mon TP.</p>`,
        code: `# Le prix de la paresse : combien de distances pour classer m nouveaux points avec n exemples ?
def nb_distances(n_exemples, m_nouveaux):
    return n_exemples * m_nouveaux

print(nb_distances(22, 8))                 # 176 : notre XV de France
print(nb_distances(150, 30))               # 4500 : les iris (120 pour apprendre, 30 à tester)
...
# Sans compter le tri des n distances pour CHAQUE nouveau point : kNN est lent à prédire.`,
        prof:
          "Le diaporama consacre 6 pages à l'implémentation scikit-learn sur les iris (lecture avec pandas, StandardScaler, KNeighborsClassifier, matrice de confusion, courbe d'erreur pour k de 1 à 40). À montrer en démonstration sur Thonny si scikit-learn est installé, pas à faire coder en Première. Le lien vers le jeu de données iris (UCI) est dans « Pour aller plus loin ».",
      },
    ],
  },
  ...
  /* ============================================================= 8 */
  {
    ...
    sections: [
      ...
      },
      {
        ...
        <p class="note">🗺️ <strong>Explorer les parcours</strong> : le site <a href="https://roadmap.sh" target="_blank" rel="noopener">roadmap.sh</a> propose des <strong>feuilles de route</strong> visuelles pour apprendre un domaine — développement <a href="https://roadmap.sh/frontend" target="_blank" rel="noopener">web</a>, <a href="https://roadmap.sh/python" target="_blank" rel="noopener">Python</a>, <a href="https://roadmap.sh/sql" target="_blank" rel="noopener">bases de données</a>, <a href="https://roadmap.sh/cyber-security" target="_blank" rel="noopener">cybersécurité</a>, <a href="https://roadmap.sh/ai-data-scientist" target="_blank" rel="noopener">IA / data</a>… Étape par étape, des bases aux compétences avancées : pratique pour voir « par où continuer » et ce que recouvre chaque métier. <em>(En anglais.)</em></p>`,
      },
      {
